...
public class IK : MonoBehaviour
{
    ...
    void Start()
    {
        if (createChains)
        {
            ...
            recursiveChainsCreation(rootNode.transform, true);
    ...
    int recursiveChainsCreation(Transform parent, bool first_chain_is_root)
    {
        ...
        Transform target = findTarget(child);
        // Debug.Log(child + " paired with " + t);
        // Debug.Log("child " + child + " | parent : " + parent_sup + " | target : " + t);
        chains.Add(new IKChain(child, parent_sup, target, first_chain_is_root));
        first_chain_is_root = false;
        ...
        if (child.childCount > 0)
        {
            ...
            {
                int idx_sub_chain = recursiveChainsCreation(child.GetChild(i), first_chain_is_root);
